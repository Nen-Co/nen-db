...
fn buildTestGraph(node_pool: *pool.NodePool, edge_pool: *pool.EdgePool) !void {
    ...
    for (cities, 0..) |city_name, i| {
        ...
        const node = pool.Node{ .id = node_id, .kind = 1, .reserved = [_]u8{0} ** 7, .props = [_]u8{0} ** 32 };
    ...
    for (routes) |route| {
        const edge = pool.Edge{ .from = route.from, .to = route.to, .kind = 1, .reserved = [_]u8{0} ** 6, .props = [_]u8{0} ** 32 };
