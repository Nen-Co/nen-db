...
pub const DistributedNode = struct {
    ...
    fn handleStats(self: *Self, _: nen_net.HttpRequest) !nen_net.HttpResponse {
        ...
        const json = try std.fmt.allocPrint(self.allocator, "{{\"nodes\":{},\"edges\":{},\"utilization\":{d:.2}}}", .{ stats.nodes.node_count, stats.nodes.edge_count, stats.utilization });
...
pub fn main() !void {
    ...
    // Add some nodes
    const node1 = try db.addNode("user", "Alice");
    const node2 = try db.addNode("user", "Bob");
    const node3 = try db.addNode("product", "Widget");

    std.debug.print("  • Added nodes: Alice, Bob, Widget\n", .{});

    // Add some edges
    try db.addEdge(node1, node2, "friends_with");
    try db.addEdge(node1, node3, "purchased");
    try db.addEdge(node2, node3, "recommended");
