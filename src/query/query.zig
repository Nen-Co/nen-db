...
fn parse_match_query(it: *std.mem.TokenIterator(u8, std.mem.DelimiterType.any), allocator: std.mem.Allocator) !Ast {
    ...
    if (pattern_token[0] == '(' and std.mem.indexOf(u8, pattern_token, ")-") != null) {
        ...
        const e_var = pattern_token[bracket_start+1..bracket_end];
        ...
        return Ast{ .Match = .{
            .pattern = Pattern{ .edge = EdgePattern{ .from = a_var, .to = b_var, .label = e_var } },
            .where = null,
        ...
        return Ast{ .Match = .{
            .pattern = Pattern{ .node = NodePattern{ .var_name = var_name, .props = null } },
            .where = where_expr,
            .ret = ReturnFields{ .fields = fields },
            .algorithm = algo,
        }};
...
fn parse_create_query(it: *std.mem.TokenIterator(u8, std.mem.DelimiterType.any), allocator: std.mem.Allocator) !Ast {
    ...
    const var_name = std.mem.trim(u8, inside[0..brace_idx], " ");
    ...
    return Ast{ .Create = .{
        .entity = Entity{ .var_name = var_name, .props = try props.toOwnedSlice() },
    }};
}
