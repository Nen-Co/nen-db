...
pub fn init(cp: u21) ?Codepoint {
    return if (@call(.always_inline, utf8.isValidCodepoint, .{cp}))
        @call(.always_inline, unsafe_init, .{cp})
    else
        null;
}
...
pub fn unsafe_init(cp: u21) Codepoint {
    return .{ .src = cp, .len = @call(.always_inline, utf8.getCodepointLength, .{cp}) };
}
...
pub fn fromUtf8(slice: []const u8) ?Codepoint {
    return if (slice.len == 0 or !@call(.always_inline, utf8.isValidSlice, .{slice})) null else @call(.always_inline, unsafe_fromUtf8, .{slice});
}
...
pub fn unsafe_fromUtf8(slice: []const u8) Codepoint {
    return if (@call(.always_inline, utf8.getSequenceLengthOrNull, .{slice[0]})) |len|
        .{ .src = utf8.decode(slice[0..len]), .len = len }
    else
        .{};
}
...
pub const Utf8Iterator = struct {
    ...
    pub fn init(slice: []const u8) ?Utf8Iterator {
        return if (slice.len == 0 or !utf8.isValidSlice(slice)) null else @call(.always_inline, Utf8Iterator.unsafe_init, .{slice});
    }
    ...
    pub fn peekCodepoint(self: Utf8Iterator) ?Codepoint {
        if (@call(.always_inline, Utf8Iterator.peekLength, .{self})) |len| {
            return Codepoint{ .src = utf8.decode(self.src[self.pos .. self.pos + len]), .len = len };
        } else return null;
    }
    ...
    pub fn peekLength(self: Utf8Iterator) ?u3 {
        return if (self.pos == self.src.len) null else @call(.always_inline, utf8.getSequenceLengthOrNull, .{self.src[self.pos]});
    }
